...
app.all('/Run', async (req, res) => {
  ...
  */
  ...
  try {
    ...
    const xml = await getFileData(url);
    console.log(`SBOL file content (${xml.length}) begins with: ${xml.slice(0, 100)}...`);
    console.log(`Parse SBOL file...`);
    const { displayList, parts } = await filesToParts(xml, {
      topLevel: top_level,
    });
    console.log('Parsed:', { displayList, parts: `Array(${parts.length})` });
...
app.listen(port, () => console.log(`App listening at http://${addr}:${port}`));
