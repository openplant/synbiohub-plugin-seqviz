...
export default async (
  ...
  try {
    const partLists = await new Promise((resolve, reject) => {
      const { fileName = '', colors = [], backbone = '', topLevel = '' } = options;
...
const fileToParts = async (
  ...
  const { fileName = '', topLevel = '', colors = [], backbone = '' } = options;
  ...
  if (file.includes('RDF')) {
    let { displayList, partLists } = await parseSBOL(file, fileName, topLevel, colors);
    // .catch(err => {
    //   console.log(err);
    //   throw err;
    // });
